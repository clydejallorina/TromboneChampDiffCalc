...
if __name__ == "__main__":
    ...
    with open("tests.csv", "w", encoding="utf-8", newline="") as csvfile:
        ...
        writer.writerow(["Song Name", "Map Diff", "Computed Diff", "Tap Rating", "Aim Rating", "TT Rating"])
        print("+-----------------------------+----------+---------------+--------------+------------+-------------+")
        print("|          Song Name          | Map Diff | Computed Diff |  Tap Rating  | Aim Rating |  TT Rating  |")
        print("+-----------------------------+----------+---------------+--------------+------------+-------------+")
        for tmb_file in os.listdir(FOLDER):
            ...
            writer.writerow([song_name, map_diff, diff_calc[0], diff_calc[2], diff_calc[1], tt_rating])
        print("+-----------------------------+----------+---------------+--------------+------------+-------------+")
    end_time = time.time()
